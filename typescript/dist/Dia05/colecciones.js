"use strict";
class ProcesoCadenasV01 {
    constructor(elementos) {
        this.elementos = elementos;
    }
    AlgunoDeLosElementosMayorQue(numeroCaracteres) {
        return this.elementos.some(el => el.length > numeroCaracteres);
    }
    AñadirNuevosElementosAlArray(cadenas) {
        if (cadenas && Array.isArray(cadenas)) {
            this.elementos.push(...cadenas.filter(el => typeof el === 'string'));
        }
    }
    DameRebanadaDeArray(indiceInicial, indiceFinal) {
        if (!this.elementos || this.elementos.length === 0)
            return [];
        if (indiceInicial < 0 || indiceFinal > this.elementos.length || indiceInicial > indiceFinal)
            return [];
        return this.elementos.slice(indiceInicial, indiceFinal);
    }
    RunningTotalsSobreLaLongitudDeLosElementos() {
        if (!this.elementos || this.elementos.length === 0)
            return 0;
        return this.elementos.reduce((acc, el) => acc + el.length, 0);
    }
    AñadirNuevoElemento(elemento) {
    ...
    ArrayConLongitudDeLosElementos() {
        return this.elementos.map(el => el.length);
    }
    ConcatenaElementosConComa() {
        return this.elementos.join(',');
    }
    IndiceDeElemento(elemento) {
    ...
    TodosLosElementosMayorQue(numeroCaracteres) {
    ...
    combinedSubarrays(lista) {
        return [...this.elementos, ...lista];
    }
    listaNombresDeLongitudX(longitud) {
...
class Padron {
    ...
    todosLongitudMayorQue(numeroCaracteres) {
